...
const Timetables: FunctionComponent<TimetablesProps> = () => {
  ...
  const getPreviousLink = () => `/station-list` +
  ...
  return (
    <div className={`${styles.pageContainer}`}>
      <Link className={`${styles.a}`} to={getPreviousLink()}><img src="/icons/go-back.svg" alt="go back"/></Link>
      <h2 className={styles.h2}>{searchParams.get(TimetableQueryParams.Line)}</h2>
      <div className={styles.container}>
        <TimeTable destination={destination} hour={12} minutes={12} departures={3} timeChange={-1} />
        <TimeTable destination={destination} hour={12} minutes={15} departures={7} timeChange={+1} />
