...
const TimeTable: FunctionComponent<TimeTableProps> = ({destination, hour, minutes, departures, timeChange}) => {
  return (
    <div className={styles.table}>
      <div className={styles.white_part}>
        <h3 className={styles.destination}>{destination}</h3>
        <div className={styles.timeData}>
          <span className={styles.time}>{hour}:{minutes}</span>
          <span
            className={`${timeChange === 0 ? styles.none : styles.changeTime}`}
            style={{color: `${timeChange < 0 ? '#34D098' : '#E85F5C'}`}}>{`${timeChange > 0 ? '+' : ''}${timeChange}`}</span>
        </div>
      </div>
