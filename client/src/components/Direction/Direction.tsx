...
const Direction: FunctionComponent<DirectionListProps> = () => {
  ...
  const getNextLink = (direction: string) => `/station-list` +
  ...
  return (
    <div className={`${styles.pageContainer}`}>
      <Link className={`${styles.a}`} to='/lines'><img src="/icons/go-back.svg" alt="go back"/></Link>
      <h2 className={`${styles.h2}`}>WSKAŻ KIERUNEK</h2>

      <div className={`${styles.buttonContainer}`}>
        <button onClick={() => { navigate(getNextLink((forth))) }} className={styles.buttonForth}>{forth}</button>
        <button onClick={() => { navigate(getNextLink((back))) }} className={styles.buttonBack}>{back}</button>
      </div>
